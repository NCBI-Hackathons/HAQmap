server <- function(input, output, session) {
  ...
  output$status <- renderText({
    if (input$goButton > 0) {
      ...
      dat$team_id <- NA #create columns we'll need later
      dat$invited <- NA
      dat$not_invited_reason <- NA
      ...
      for (i in 1:length(unique(dat$team))){
        ...
        #create the url
        url <- paste("https://slack.com/api/channels.create?token=", input$api_token, "&name=", team, sep="")
        #make the call
        resp <- POST(url)
